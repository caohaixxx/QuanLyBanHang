...
class ShopController extends Controller
{
    ...
    public function filter($products, Request $request)
    {
        ...
        $proCate = ProductCategory::select(
            'pro_cates.id',
            'pro_cates.product_id',
            'pro_cates.category_id',
            ...
            DB::raw('MAX(product_details.price) as maxPrice'),
            DB::raw('MIN(product_details.price) as minPrice'),
        )->join('products', 'products.id', 'pro_cates.product_id')
        ->join('categorys', 'categorys.id', 'pro_cates.category_id')
        ->leftjoin('product_details', 'products.id', 'product_details.product_id')
        ->whereIn('category_id', $cate_ids)
        ->groupBy('products.id');

        $products = $cate_ids !=null ? $proCate : $products;
